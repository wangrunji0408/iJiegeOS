...
fn fork_task(parent: &Arc<Task>, flags: usize, child_sp: usize, ptid: usize, ctid: usize, newtls: usize, parent_ctx: &TrapContext) -> Task {
    ...
    *trap_cx = *parent_ctx;
    ...
    // kernel_satp = 0（内核没有独立页表，陷阱时不切换 satp）
    trap_cx.kernel_satp = 0;
    // 子进程 fork 返回 0
    ...
    log::error!("[fork] pid={}: trap_cx_addr={:#x} kernel_sp={:#x} sepc={:#x} user_sp={:#x} user_satp={:#x} sstatus={:#x}",
        pid.0, trap_cx_addr, kernel_sp, trap_cx.sepc, trap_cx.get_sp(), trap_cx.user_satp, trap_cx.sstatus);

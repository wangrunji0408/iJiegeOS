...
pub fn dispatch(id: usize, args: [usize; 6], _cx: &mut TrapContext) -> isize {
    ...
    if TRACE.load(core::sync::atomic::Ordering::Relaxed) {
        ...
        crate::println!("[sc] {}({}) args={:#x?} ret=...", name, id, args);
    }
    let r = dispatch_inner(id, args, _cx);
    if TRACE.load(core::sync::atomic::Ordering::Relaxed) {
        crate::println!("[sc]   {} -> {}", id, r);
    }
    r
}
...
fn dispatch_inner(id: usize, args: [usize; 6], _cx: &mut TrapContext) -> isize {
    match id {
        ...
        SYS_FCHOWNAT | SYS_FCHOWN => 0,
        SYS_FCHMODAT => 0,
        SYS_UNLINKAT | SYS_SYMLINKAT | SYS_LINKAT | SYS_RENAMEAT | SYS_RENAMEAT2 => 0,
        SYS_TRUNCATE | SYS_FTRUNCATE | SYS_FALLOCATE => 0,
        SYS_STATFS | SYS_FSTATFS => 0,
        SYS_UTIMENSAT => 0,
        SYS_FSYNC | SYS_SYNC | SYS_MSYNC => 0,
